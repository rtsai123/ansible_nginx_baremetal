...
MOLECULE := $(PWD)/.venv/bin/molecule
...
deps:
	...
	@$(PIP) install "ansible>=9,<10" "ansible-core>=2.16.0,<2.17" "molecule>=5" "molecule-plugins[docker]" ansible-lint
...
.PHONY: provision-local provision-remote-pwd provision-key rerun-ansible
provision-local:
	@echo "🚀 Running Ansible against Vagrant VM..."
	. $(VENV)/bin/activate && $(ANSIBLE) -i inventory.ini site.yml -e target_hosts=vagrant -v

provision-remote-pwd:
	@echo "🌐 Deploying to remote Ubuntu servers..."
	. $(VENV)/bin/activate && $(ANSIBLE) -i inventory.ini site.yml -e target_hosts=remote_servers_pwd -v

provision-remote-key:
	@echo "🌐 Deploying to remote Ubuntu servers..."
	. $(VENV)/bin/activate && $(ANSIBLE) -i inventory.ini site.yml -e target_hosts=remote_servers_key -v
...
molecule-test:
	@test -x $(MOLECULE) || (echo "❌ Molecule not found. Run 'make deps' first."; exit 1)
